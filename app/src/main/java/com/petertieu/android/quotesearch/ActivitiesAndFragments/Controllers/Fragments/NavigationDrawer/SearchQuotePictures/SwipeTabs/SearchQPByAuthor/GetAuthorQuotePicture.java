package com.petertieu.android.quotesearch.ActivitiesAndFragments.Controllers.Fragments.NavigationDrawer.SearchQuotePictures.SwipeTabs.SearchQPByAuthor;
...
public class GetAuthorQuotePicture {


    private static final String TAG = "GRQPictureQuote"; //Log to Logcat

    //Fetch the fields mentioned above
    //NOTE: Multiple AsyncTasks would be run simultaneously in AuthorQuotePicturesFragment. Thus, this method would be run multiple times simultaneously.
    public QuotePicture getAuthorQuotePictureQuote(String searchQuery){

        QuotePicture authorQuotePicture = new QuotePicture(); //Instantiate a QuotePicture

        String urlString = "http://quotes.rest/quote/image/search.json?" + "author=" + searchQuery + "&api_key=1FGGcyK9BwzYfAi8IyYZ8geF"; //Define URI endpoint for networking
        //Fetches JSON text of a Author Quote Picture containing:
        //1: ID
        ...
        return authorQuotePicture; //Return the QuotePicture
    }
    ...
    private void parseJSONObjectToAuthorQuotePictureQuote(QuotePicture authorQuotePicture, JSONObject jsonObject) throws IOException, JSONException {
