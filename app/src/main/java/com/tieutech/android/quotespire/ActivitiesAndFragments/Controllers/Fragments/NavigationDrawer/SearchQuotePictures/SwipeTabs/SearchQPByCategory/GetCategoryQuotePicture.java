...
//Uses networking with the TheySaidSo API to fetch the JSON file of a Quote Picture/Image, and stash the following fields to a newly created QuotePicture object:
    //1: Quote ID
    //2: Picture Download URI (which will be passed to the HandlerThread to obtain the QuotePicture Picture)
//This class is called by the AsyncTask of SearchQPByCategoryFragment. Thus, it runs asynchronously to the main thread
public class GetCategoryQuotePicture {

    private static final String TAG = "GRQPictureQuote"; //Log to Logcat

    //Fetch the fields mentioned above
    //NOTE: Multiple AsyncTasks would be run simultaneously in CategoryQuotePicturesFragment. Thus, this method would be run multiple times simultaneously.
    public QuotePicture getCategoryQuotePictureQuote(String searchQuery){
        ...
        //Try risky task - getJSONString(..) and parseJSONObjectToCategoryQuotePictureQuote(..) may throw IOException and JSONException, respectively
        try {
            ...
            parseJSONObjectToCategoryQuotePictureQuote(categoryQuotePicture, JSONObject); //Parse the JSON Object to the QuotePicture
        }
        ...
        return categoryQuotePicture; //Return the QuotePicture
    }
    ...
    private void parseJSONObjectToCategoryQuotePictureQuote(QuotePicture categoryQuotePicture, JSONObject jsonObject) throws IOException, JSONException {
        ...
        categoryQuotePicture.setId(qImageObject.getString("quote_id")); //Obtain the quote ID and stash it to the Category QuotePicture
        categoryQuotePicture.setQuotePictureDownloadURI(qImageObject.getString("download_uri")); //Obtain the quote Picture Download URI and stash it ot the Category QuotePicture


        Log.i(TAG, "Category Quote Picture Quote - method - Quote ID: " + categoryQuotePicture.getId()); //Log ID
        Log.i(TAG, "Category Quote Picture Quote - method  - Picture Download URI: " + categoryQuotePicture.getQuotePictureDownloadURI()); //Log Picture Download URI
    }
